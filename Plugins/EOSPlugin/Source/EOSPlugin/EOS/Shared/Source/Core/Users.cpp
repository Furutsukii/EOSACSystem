...
void FUsers::QueryUserInfo(EOS_EpicAccountId TargetUserId, std::function<void(const FUserData_EOS&)> UserInfoRetrievedCallback)
{
	PlayerPtr Player = FPlayerManager::Get().GetPlayer(FPlayerManager::Get().GetCurrentUser());
	if (Player != nullptr)
	{
...
void FUsers::QueryUserInfo(EOS_EpicAccountId LocalUserId, EOS_EpicAccountId TargetUserId, std::function<void(const FUserData_EOS&)> UserInfoRetrievedCallback)
{
	QueryUserInfo(LocalUserId, TargetUserId, QueryUserInfoCompleteCallbackFn, UserInfoRetrievedCallback);
}

void FUsers::QueryUserInfo(EOS_EpicAccountId LocalUserId, const std::wstring& DisplayName, std::function<void(const FUserData_EOS&)> UserInfoRetrievedCallback)
{
	QueryUserInfo(LocalUserId, DisplayName, QueryUserInfoByDisplayNameCompleteCallbackFn, UserInfoRetrievedCallback);
}

void FUsers::QueryUserInfo(EOS_EpicAccountId LocalUserId, EOS_EpicAccountId TargetUserId, const EOS_UserInfo_OnQueryUserInfoCallback CompletionDelegate, std::function<void(const FUserData_EOS&)> UserInfoRetrievedCallback)
{
...
void FUsers::QueryUserInfo(EOS_EpicAccountId LocalUserId, const std::wstring& DisplayName, const EOS_UserInfo_OnQueryUserInfoByDisplayNameCallback CompletionDelegate, std::function<void(const FUserData_EOS&)> UserInfoRetrievedCallback)
{
	EOS_HUserInfo UserInfoInterface = EOS_Platform_GetUserInfoInterface(FPlatform::GetPlatformHandle());
...
void EOS_CALL FUsers::QueryUserInfoCompleteCallbackFn(const EOS_UserInfo_QueryUserInfoCallbackInfo* UserData)
{
	...
	if (UserInfo && Result == EOS_EResult::EOS_Success)
	{
		...
		FUserData_EOS RetrievedUserData;
...
void EOS_CALL FUsers::OnQueryDisplayNameFinishedCallback(const FUserData_EOS& UserData)
{
	if (UserData.IsValid())
	{
