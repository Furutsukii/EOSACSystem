...
void FSettings::Init()
{
	std::wstring CmdFileName = EOS_CommandLine::FCommandLine::Get().GetParamValue(CommandLineConstants::SettingsFile);
...
FSettings::SettingsEntryType FSettings::GetSettingsEntryTypeFromString(std::wstring ValStr)
{
	...
	if (ValStr.find(L".") != std::wstring::npos)
	{
		...
		//		{
		size_t Size(0);
		float fTemp = std::stof(ValStr, &Size);
...
//	{
	size_t Size(0);
	int iTemp = std::stoi(ValStr, &Size);
